...
def main():
    ...
    marker_id1, marker_id2 = 21, 23  # Those are the markers on the left
    ...
    virtual_point = (-20, 130)  
    virtual_point2 = (-30, 125)  
    ...
    while True:
        ...
        if ids is not None:
            ...
            # Check if we have both required markers
            if marker_id1 in marker_positions and marker_id2 in marker_positions:
                ...
                try:
                    # Find the inverse transformation (from desired to pixel coordinates)
                    inverse_transform = compute_transformation_matrix(dst_points, src_points)

                    # Transform the virtual point to pixel coordinates
                    transformed_point = apply_transformation(virtual_point, inverse_transform)
                    transformed_point2 = apply_transformation(virtual_point2, inverse_transform)

                    # Draw the virtual points on the frame
                    #TODO: put virtual points in a list and do a loop to draw them
                    ...
                    cv2.circle(undistorted_frame, (int(x2), int(y2)), 10, (0, 255, 0), -1)
                    cv2.putText(undistorted_frame, f"Virtual Point ({virtual_point2[0]}, {virtual_point2[1]})",
                                (int(x2) + 10, int(y2)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        ...
        # Display information
        cv2.putText(undistorted_frame, "Looking for markers 1 and 2", (10, 30),
                  cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
